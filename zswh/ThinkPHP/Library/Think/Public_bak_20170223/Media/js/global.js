    ...
    (function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                   ...
                   case 1:
						...
						$('#topShowName').text('您好，' + data.account+'【个人中心】').attr('href',UURL +'index');;
						return;
                        break;
                    default:
		...
		XGUC.phone_reg = function(form_id) {
            ...
            $('#' + form_id).unbind('submit').bind('submit',function(event) {
				...
				truename = $.trim($('#phonetruename').val());
				idcard = $.trim($('#phoneidcard').val());
                ...
                if (pwd.length<6 || pwd.length>30){f('#registerPhonePassPop','6~30位数字、字母或特殊字符组成');return false;}
                if (truename == ''){f('phonetruename','真实姓名不能为空',false);return false;}
                if (idcard == ''){f('phoneidcard','身份证号码不能为空',false);return false;}
		...
		XGUC.name_reg = function(form_id) {
            ...
            $('#' + form_id).unbind('submit').bind('submit',function(event) {
                ...
                if (truename == ''){f('truename','真实姓名不能为空',false);return false;}
                if (idcard == ''){f('idcard','身份证号码不能为空',false);return false;}
...
function checkIdcard(card_id){

	var idcard = $("#"+card_id).val();
	if(idcard==""){
		ShowMsg(card_id,0,"身份证号码不能为空 !");
	...
	if (!(/(^\d{15}$)|(^\d{17}([0-9]|X)$)/.test(idcard))){
		...
		ShowMsg(card_id, 0, "输入的身份证号码格式错误");
		//jQuery("#idcardspan").attr("class", "error");
		return false;
	}
	...
	if (len == 15){
		...
		if (!bGoodDay){
			//jQuery('#idcardspan').show();
			ShowMsg(card_id, 0, "输入的身份证号里出生日期不对！");
			//jQuery("#idcardspan").attr("class", "error");
			return false;
		}else{
			//将15位身份证转成18位
...
//姓名验证
function checkTruename(name_id){

	var truename = $("#"+name_id).val();
	//var website = $("#website").val();
	if(truename==""){

		ShowMsg(name_id,0,"真实姓名不能为空 !");
		//jQuery("#truenamespan").attr("class","error");
		return false;
	}
	...
	if(truename.length >=2 && truename.length <= 4){
		...
		$.ajax({
			...
			url: url,
			async: false,
			data:"do=truename"+"&truename=" + truename,
			dataType:"json",
			error:function(d,t){
				ShowMsg("truenamespan",0,"网络错误");
				isCanTruename=false;
			},
			success: function(data){
				if(data == 1){
					isCanTruename = true;
					$('#truenamespan').hide();
					ShowMsg("truenamespan",1,"姓名确认无误。");
					//jQuery("#truenamespan").attr("class","ok");
				}else if (data == 2){
					//jQuery('#truenamespan').show();
					ShowMsg("truenamespan", 0, "该名已经被使用");
					//jQuery("#truenamespan").attr("class", "error");
					isCanTruename = false;
					return false;
				}
