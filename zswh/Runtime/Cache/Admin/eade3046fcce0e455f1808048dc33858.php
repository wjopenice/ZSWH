...
<body>
	...
	<div class="cf top_nav_list">
        ...
        <div class="fl button_list">
            <a class="btn red" id="dtnn" href="javascript:;">批量删除</a>
		</div>
        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			...
			<div class="input-list" id='newQd' >
				<input type="text" name="<?php echo ((isset($model['search_key']) && ($model['search_key'] !== ""))?($model['search_key']):'promote_account'); ?>" class="search-input" value="<?php echo I('promote_account');?>" placeholder="请输入渠道账号">
				<!-- <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('propay/lists','model='.$model['name'],false);?>"><i class="btn-search"></i></a> -->
			</div>
       ...
       <!--      <div class="i_list">
             ...
             <div class="chose_game"  id="sel_game" style="float:left;width:172px;height:28px;line-height:28px;margin:0 5px 0 5px;">
                        <select>
                            <?php if(I('game_name') == ''): ?><option value="" >请选择游戏</option>
                            <?php else: ?>
                                <option value="<?php echo I('game_name');?>">请选择游戏</option><?php endif; ?>                                                     
            ...
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('BangPropay/promote_game_list','model='.$model['name'],false);?>">搜索</a>
            </div>
            <div class="input-list">
                <a class="sch-btn" href="<?php echo U('Export/expUser',array('id'=>17,'game_name'=>I('game_name'),'time-start'=>I('time-start'),'time-end'=>I('time-end'),'start'=>I('start'),'end'=>I('end'),));?>">导出</a>
            </div>
		</div>
	</div>
    ...
    <!-- <span><a href="<?php echo U('Export/expUser',array( 'id'=>9, 'game_name'=>I('game_name'), 'time-start'=>I('time-start'), 'time-end'=>I('time-end'), 'start'=>I('start'), 'end'=>I('end'), ));?>">导出数据(excel格式)</a></span> -->
...
<style>
   ...
   #sel_game .text-input{height:28px!important;}
...
<script >
$(function(){
    var gameBh="<?php echo ((isset($_GET['game_name']) && ($_GET['game_name'] !== ""))?($_GET['game_name']):0); ?>";
...
//导航高亮
highlight_subnav('<?php echo U('BangPropay/promote_game_list');?>');
$(function(){
     $('.ids').ready(function(){
	...
	$("#search").click(function(){
        ...
        var query  = $.param({           
            'game_name':$.trim($("#sel_game select").val()),
            'promote_account':$.trim($("#newQd input").val())
        });
     ...
     var spCodesTemp = "";
      ...
      if(spCodesTemp==""){
        ...
        $(this).attr('href','/admin.php?s=/BangPropay/batch/ids/'+spCodesTemp);
      }
...
$('#time-start').datetimepicker({
  ...
  var spCodesTemp = "";
      ...
      if(spCodesTemp==""){
        ...
        $(this).attr('href','/admin.php?s=/BangPropay/delprovide/ids/'+spCodesTemp);
      }
