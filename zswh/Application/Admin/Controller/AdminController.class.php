...
class AdminController extends Controller {
    ...
    protected function _initialize(){
        ...
        if(!IS_ROOT){
            ...
            if ( false === $access ) {
               ...
               // if ( !$this->checkRule($rule,array('in','1,2')) ){
                 if (!$this->checkRule($rule, array('in', '1,2'))) {
                     if($rule!="admin/repair/repair_ajax_data" && $rule!="admin/repair/repair_ajax") {
                         $this->error('未授权访问!');
                     }
                    } else {
                        // 检测分类及内容有关的各项动态权限
        ...
        $web_site_title = M("config")->where("id=1")->find();
        $this->assign('web_site_title',$web_site_title['value']);
