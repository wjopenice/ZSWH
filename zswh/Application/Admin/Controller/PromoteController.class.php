...
class PromoteController extends ThinkController {
    ...
    public function add($account=null,$password=null,$second_pwd=null,$real_name=null,$email=null,$mobile_phone=null,$bank_name=null,$bank_card=null,$admin=null,$status=null){
        if(IS_POST){
           ...
           $this->assign('user_id',$_SESSION['onethink_admin']['user_auth']['uid']);
            $this->assign('user_name',$_SESSION['onethink_admin']['user_auth']['username']);
            $this->display();
        }
    }
    ...
    */
    public function ch_reg_list(){                                                   //游戏名称要么全部
        ...
        if($auth_group_id!=1)
        {
            $admin_ids=M('promote','tab_')->where('admin_id='.$uid)->select();
            $admin_id='';
            ...
            if(!empty($admin_id)) {
                $map['tab_user.promote_id'] = array('in', $admin_id);
    ...
    */
    public function alipay_list(){
      ...
      public function branch()
      {
          ...
          $data=$model
              ->field('tab_promote.id,tab_promote.account,tab_promote.parent_id,tab_promote.real_name,tab_promote.mobile_phone,tab_promote.status,tab_promote.alipayway_sign,admin_id')
             // ->join("sys_member ON tab_promote.admin_id = sys_member.uid ")
              ->where($map)
              ->order('id desc')
              ->page($page, 10)
              ->select();
            foreach ($data as $key=>$value)
            {
                ...
                $member=M('member','sys_')->field('nickname')->where($where)->find();
                $data[$key]['nickname']=$member['nickname'];
            }
          //print_r($data);exit;
          $count=$model
              ->field('tab_promote.id,tab_promote.account,tab_promote.parent_id,tab_promote.real_name,tab_promote.mobile_phone,tab_promote.status,tab_promote.alipayway_sign,sys_member.nickname')
             // ->join("sys_member ON tab_promote.admin_id = sys_member.uid ")
              ->where($map)
              ->count();
          if($count > $row){
      ...
      public function build($id){
          ...
          {
              $branchgame=M('branch_game','tab_');
              $branchgame->where($where)->delete();
              $newdata['admin_id']=$uid;
              $newdata['promote_id']=$_REQUEST['promote_id'];
              $newdata['promote_account']=$_REQUEST['promote_account'];
              $game_ids= substr($_REQUEST['game_ids'],0,strlen($_REQUEST['game_ids'])-1);
              $game_names= substr($_REQUEST['game_names'],0,strlen($_REQUEST['game_names'])-1);
              $gameids=explode(',', $game_ids);
              $gamenames=explode(',', $game_names);
                 foreach ($gameids as $key=>$value)
                {
                    $newdata['game_id']=$value;
                    $newdata['game_name']=$gamenames[$key];
                    $res=$branchgame->add($newdata);
                    if(!$res)
                    {
                       echo $branchgame->getError();
                    }
                }
                 echo 1;
              exit;
          ...
          $model = D('Promote');
          $data = $model->find($id);
          $this->assign('promote_account',$data['account']);
          $this->assign('promote_id',$id);
          $result=M('branch_game','tab_')->field("game_id,game_name")->where($where)->select();
          $this->assign('result',$result);
          $this->display();
      }
      public function branchgame(){
          $uid=$_SESSION['onethink_admin']['user_auth']['uid'];
          echo $uid;exit;
      }
