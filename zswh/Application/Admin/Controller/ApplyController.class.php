...
class ApplyController extends ThinkController {
    ...
    public function package($ids=null,$type=null) //3311
    {
        ...
        try{
            ...
            //要显示的一段html的字符串
            $html = "
<div id='loadcss' style='position:relative;height: 100%;'>
<img style='position: absolute;left: 50%;margin-left: -280px;top: 50%;margin-top: -360px;' src='/Public/Admin/images/timg.gif' />
<p id='loadtext' style='font-size: 34px;text-align: center;margin-top: 10px;'>正在打包，请稍等......</p>
</div>";
            echo $html;
            ...
            if ($res === TRUE) {
                ...
                $jieguo = $this->updateinfo($ids,$to,$apply_data,$type);
                if($jieguo){
                    $script = '<script>document.getElementById("loadtext").innerText="打包完成";</script>';
                    echo $script;
                    echo ob_get_clean();    //获取当前缓冲区内容并清除当前的输出缓冲
                    flush();   //刷新缓冲区的内容，输出
                    echo "<script>window.location.href='/admin.php?s=/Apply/lists.html';</script>";
                    exit;
                }
                else{
                    $script = '<script>document.getElementById("loadtext").innerText="打包失败";</script>';
                    echo $script;
                    echo ob_get_clean();    //获取当前缓冲区内容并清除当前的输出缓冲
                    flush();   //刷新缓冲区的内容，输出
