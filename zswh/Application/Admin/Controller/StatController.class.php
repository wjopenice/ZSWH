 ...
 */
    ...
    */
    ...
    public function userarpu()
    {
        ...
        $paycount=$this->getpaycount2();
        $allpay=$this->getallpaycount2();
        $newpaycount=$this->getnewpaycount2();
        $cilogin = $this->get_cilogin($newcount,$cicount);
        $allrate= round($this->getrate2(),3);
        $userarpu=round($this->getuserarpu2(),3);
        $payarpu= round($this->getpayarpu2(),3);
    ...
    //计算新用户付费金额2
    public function getnewpaycount2(){
            $count=0;
            if(isset($_REQUEST['time-start'])&&isset($_REQUEST['game_name'])){
                if($_REQUEST['game_name']=='全部'){
                    unset($_REQUEST['game_name']);
            ...
            if(!empty($map)){
                $list=M("User as a","tab_")
                    ->field("sum(pay_amount) as sum")
                    ->join("tab_spend as c on c.user_id=a.id")
                    ->where($map)
                    ->find();
                if(!empty($list['sum'])){
                    $count=$list['sum'];
    ...
    //计算总付费金额
    public function getallpaycount2(){
        $count=0;
        if(isset($_REQUEST['time-start'])&&isset($_REQUEST['game_name'])){
            if($_REQUEST['game_name']=='全部'){
                unset($_REQUEST['game_name']);
                ...
                $map['c.game_id']=get_game_id($_REQUEST['game_name']);
                $map['pay_status']=1;
                $map['c.pay_time']=get_start_end_time($_REQUEST['time-start']);
            }
        }
        if(!empty($map)){
            $list=M("User as a","tab_")
    ...
    //获取用户ARPU
    public function getuserarpu2()
    {
        $new=$this->getnewpaycount2();
        if(isset($_REQUEST['time-start'])){
            $newcount=end($this->getnewcount($_REQUEST['time-start']));
            $count=$new/$newcount;
        }else{
    ...
    //获取付费ARPU
    public function getpayarpu2()
    {
        $paysum=$this->getallpaycount2();
        $paycount=$this->getpaycount2();
        if($paycount!=0){
            $count=$paysum/$paycount;
        }else{
