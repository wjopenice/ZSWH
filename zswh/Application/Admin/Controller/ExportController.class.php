     	...
     	switch ($id) {
          ...
          case 12:
            ...
            break;
             case 23:
                ...
                $xlsCell  = array(
                 ...
                 if(isset($_REQUEST['qu_id'])){
                     $map['id']=$_REQUEST['qu_id'];
                     unset($_REQUEST['qu_id']);
                 }
                $ids=$_REQUEST['ids'];
                if(isset($_REQUEST['account'])){
            ...
            case 32:
                ...
                $xlsData=M('BindSpend','tab_')
                    ->field("id,user_account,promote_account,game_id,game_name,extend,pay_amount,pay_order_number,order_number,FROM_UNIXTIME(pay_time,'%Y-%m-%d %H:%i:%s') as period")
                   ->where($map)
                    ->order('pay_time desc')
                    //根据字段分组
                     ->where($map)
                    ->select();
               //var_dump(M('BindSpend','tab_')->getlastsql());
              // exit;
                break;
