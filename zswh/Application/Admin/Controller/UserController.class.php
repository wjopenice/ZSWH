...
class UserController extends AdminController {
     ...
     * @author 麦当苗儿 <zuojiazi@vip.qq.com>
     */
    public function filter($p = 0){
        $row=10;
        if(isset($_REQUEST['keywords'])){
            $map['word']= array('like', '%'.(string)$_REQUEST['keywords'].'%');
        ...
        $data =M("filter","tab_")
            ...
            ->select();
        /* 查询记录总数 */
        $count = M("filter","tab_")->where($map)->count();
        //分页
        if($count > $row){
    ...
    public function del_filter()
    {
       ...
       empty($ids) && $this->error('参数错误！');
        if(is_array($ids)){
            ...
            {
                $map['id'] = $val;
                $res = M('filter','tab_')->where($map)->delete();
            }
