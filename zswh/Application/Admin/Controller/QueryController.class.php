...
class QueryController extends ThinkController {
    ...
    */
    public function bill($p = 0){
        ...
        $user = A('Spend','Event');
    	$pay_amount2=D('spend')->where($map)->sum('pay_amount');
    	$this->assign('pay_amount2', $pay_amount2);
        $user->group_list($model,$p,$map);
    }
    public function settlement($p=1){
        $row=10;
        if(isset($_REQUEST['game_name'])){
    ...
    public function spend($p = 0){
       ...
       /* if(isset($_REQUEST['promote_name'])){
        ...
        $data = D('bind_spend')
            ...
            ->select();
          /* 查询记录总数 */
        $count = D('bind_spend')->where($map)->count();
        //分页
        if($count > $row){
            $page = new \Think\Page($count, $row);
            $page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %HEADER%');
        ...
        $pay_amount2=D('bind_spend')->where($map)->sum('pay_amount');
        $this->assign('pay_amount2', $pay_amount2);
        $this->assign('list_data', $data);
        $this->display();
    }
