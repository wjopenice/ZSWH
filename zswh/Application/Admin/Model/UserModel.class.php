...
class UserModel extends Model{
    ...
    //user表新加game_id 如果没有数据,新加数据
    public function login2_($account,$type,$game_id,$game_name,$sdk_version){
        $map['phone'] = $account;
        /* 获取用户数据 */
        $user = $this->where($map)->find();
        if(is_array($user) && $user['lock_status']){
    ...
    */
    ...
    public function register2_($account,$password,$register_way,$promote_id=0,$promote_account="",$phone="",$game_id="",$game_name="",$sdk_version=""){
        $data = array(
            'account'    => $account,
            'password'   => $password,
            'nickname'   => $account,
        ...
        /* 添加用户 */
        $user = $this->where(['phone'=>$account])->find();
