...
class UserController extends BaseController{
    ...
    */
     ...
     * SDK用户手机号验证码登录
     */
    public function user_phone_login(){
        //获取SDK上POST方式传过来的数据 然后base64解密 然后将json字符串转化成数组
        $user = json_decode(base64_decode(file_get_contents("php://input")),true);
        $gameid = $user['game_id'];
        $phone = $user['phone'];
        ...
        $game_appid = $user['game_appid'];
        $promote_id = $user['promote_id'];
        $promote_account = $user['promote_account'];
        //验证短信验证码
        $this->sms_verify2($phone,$sms_code);
        //实例化用户接口
        $userApi = new MemberApi();
        $userdata = M("user","tab_")->where("phone = '{$phone}'")->find();
        if(!empty($userdata)){
            $user['password'] = $userdata['password'];
            //用户存在登录
            $result = $userApi->login2_($phone,1,$gameid,$game_name);
            $res_msg = array();
            switch ($result) {
                case -1:
                    $this->set_message(-1,"fail","用户不存在或被禁用");
            ...
            //用户不存在注册用户
            $result = $userApi->register2_($phone,$password,1,$promote_id,$promote_account,$phone,$gameid,$game_name);
            $res_msg = array();
            if($result > 0){
                session($phone,null);
                $map['phone'] = $phone;
                ...
                $this->set_message(-3,"fail","系统繁忙，请稍候再试");
            }
        }
    }
    /**
     * SDK用户优惠券列表
     */
    public function user_coupon_record(){
        $data = json_decode(base64_decode(file_get_contents("php://input")),true);
        $time=strtotime(date("Y-m-d 00:00:00"));
        $model=M('user_coupon','tab_');
        $result=$model
            ->alias('a')
            ->field('a.coupon_id,b.userule,b.amount,b.credit,b.start_time,b.end_time ')
            ->join("tab_coupons as b on a.coupon_id = b.id ")
            ->where("a.user_id=".$data['user_id']." and a.status=0 and b.start_time<=$time and b.end_time>=$time")
    ...
    */
    public function send_sms()
    {
        ...
        $xigu = new Xigu($smsconfig);
        $param = $rand.",".'1';
        $result = json_decode($xigu->sendSM(C('sms_set.smtp_account'),$phone,$smsconfig['sms_set']['smtp_password'],$param),true);

        file_put_contents("./log/sms.txt","sms: ".json_encode($result)."\r\n",FILE_APPEND);
        ...
        $zs_short_message['pid'] = 0;
        $status = ($result['code'] == 200) ? 1 : 0;
        $zs_short_message['status'] = $status;
        $zs_short_message['ratio'] = 0;

        file_put_contents("./log/sms.txt","sms: ".json_encode($zs_short_message)."\r\n",FILE_APPEND);
        try {
            M('Short_message',"tab_")->add($zs_short_message);
        } catch (\Exception $e) {
            file_put_contents("./log/sms.txt","sms: ".json_encode(['sql'=>'error'])."\r\n",FILE_APPEND);
        }
    ...
    */
     ...
     * sdk用户游戏充值记录
     */
    public function spend_recond_list(){
        //获取SDK上POST方式传过来的数据 然后base64解密 然后将json字符串转化成数组
        $data = json_decode(base64_decode(file_get_contents("php://input")),true);
        $model=M('Spend','tab_');
        $payArr = $model->field('pay_time,pay_amount,pay_way,pay_status,props_name,pay_order_number')->where("user_id=".$data["user_id"]." and game_id=".$data["game_id"])->select();
        if(!empty($payArr)){
            $listArr['status'] = 1;
            $listArr['list'] = $payArr;
        }else{
    ...
    public function user_deposit_list(){
        //获取SDK上POST方式传过来的数据 然后base64解密 然后将json字符串转化成数组
        $data = json_decode(base64_decode(file_get_contents("php://input")),true);
        $model=M('Deposit','tab_');
        $payArr = $model->field('id,user_account,pay_order_number,pay_amount,pay_status,create_time')->where(" user_id=".$data["user_id"] )->select();
        if(!empty($payArr)){
            $listArr['status'] = 1;
            $listArr['list'] = $payArr;
        }else{
