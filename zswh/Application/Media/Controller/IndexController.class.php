...
class IndexController extends BaseController {
    ...
    public function __construct() {
        ...
        if(isMobile()){
            header("Location:/Mobile/Index/wapindex.html");
        }
    ...
    public function get_games() {
        ...
        //休闲益智
        $xiuxian = $this->get_game_by_type($game_list, 'game_type_id', '16', 8);
...
//    }
    ...
    public function pettap(){
        header('Access-Control-Allow-Origin:*');//允许所有来源访问
        header('Access-Control-Allow-Headers:content-type');
        header('Access-Control-Allow-Method:POST');//允许访问的方式
        $arrData = M()->query("SELECT id,icon,game_name,introduction,and_dow_address FROM tab_game WHERE id IN (446,450,452,444)");
        $reco = [$arrData[1],$arrData[2],$arrData[3],$arrData[0]];
        foreach ($reco as $k1=>$v1){
            $reco[$k1]['icon'] = get_cover($v1['icon'])['path'];
            $reco[$k1]['and_dow_address'] = substr($v1['and_dow_address'],1) ;
        }
        $arrData1 = M()->query("SELECT id,icon,game_name,introduction,and_dow_address FROM tab_game WHERE id IN (443,434,453)");
        $hot = [$arrData1[1],$arrData1[0],$arrData1[2]];
        foreach ($hot as $k2=>$v2){
            $hot[$k2]['icon'] = get_cover($v2['icon'])['path'];
            $hot[$k2]['and_dow_address'] = substr($v2['and_dow_address'],1) ;
        }
        $xin = M()->query("SELECT id,icon,game_name,introduction,and_dow_address FROM tab_game WHERE id NOT IN (446,450,452,444,443,434,453) AND cp_name <> '多酷游戏合作联盟' AND game_status = 1");
        foreach ($xin as $k3=>$v3){
            $xin[$k3]['icon'] = get_cover($v3['icon'])['path'];
            $xin[$k3]['and_dow_address'] = substr($v3['and_dow_address'],1) ;
        }
        //推荐游戏
        $data['reco'] = $reco;
        //热门游戏
